...
**Kondisi pada 2026-08-29 (Addendum, evidence terverifikasi langsung pada sesi ini):** Kondisi di atas **sudah berubah**. Engine backup dan restore verification telah diimplementasikan (Sprint 2, commit `949a3e4f`, 2026-08-16) dan diuji unit secara otomatis di CI (`ci-generic.yml`, job `db:backup:test` dan `db:backup:uploads:test`, DB-independent, berjalan setiap push/PR ke `main`). Backup nyata dan restore verification nyata dijalankan langsung terhadap `db_epelara` lokal/dev dan **berhasil** — lihat §9 untuk detail evidence.

**Kondisi pada 2026-08-29, Addendum kedua (penjadwalan otomatis berkala kini operasional):** Project Owner mendaftarkan Windows Scheduled Task ("ePeLARA - Database Backup", harian 02:00), diverifikasi berjalan nyata (evidence lengkap di §9). RPO 24 jam kini **sepenuhnya operasional**, bukan hanya *capable* — backup harian berjalan otomatis tanpa campur tangan manual berkelanjutan.
...
| **Bukti restore test aktual (untuk closure formal AIR-009)** | **Tersedia.** Dieksekusi langsung pada sesi ini, 2026-08-29, terhadap `db_epelara` **lokal/dev** (bukan server produksi — lihat catatan di bawah): `backup_id=bkp_1787962698693_9de24c3c` (87.070.766 bytes, SHA-256 tercatat di manifest) → `db:restore:verify` → outcome **`RESTORE_VERIFIED`**, `checksum_match: true`, `tables_expected: 283` / `tables_found: 283`, seluruh critical tables (`users`, `roles`, `divisions`, `periode_rpjmds`) hadir, `cleanup_status: SUCCESS` (database temporary `epelara_restore_verify_1787962712287_cca7cad7` berhasil di-DROP, tidak pernah menyentuh `db_epelara`). Log lengkap tersimpan di `backups/database/restore-verify.log.jsonl` pada mesin ini. **Keputusan Closure Project Owner (2026-08-29)**: "Tutup AIR-009: terima bukti backup+restore yang ada sebagai cukup untuk penutupan resmi. Syaratnya: restore test wajib diulang secara berkala (misal tiap bulan), bukan sekali saja." AIR-009 kini **Closed** di Architecture Issue Register (v1.0.17) — bukti sekali-jalan di lokal/dev diterima cukup untuk closure formal, dengan syarat operasional berkelanjutan (bukan pre-condition closure): restore test wajib diulang berkala. **Addendum ketiga (2026-08-29, syarat berkelanjutan kini operasional)**: Project Owner mendaftarkan Windows Scheduled Task kedua ("ePeLARA - Restore Verify", trigger mingguan Minggu 03:00, `LogonType=S4U` + `AllowStartIfOnBatteries` sejak awal — sudah menerapkan langsung pelajaran dari diagnosis task backup sebelumnya, tidak mengalami kegagalan senyap). Diverifikasi independen: dipicu manual, ditunggu sampai `LastTaskResult` benar-benar `0` (bukan `267009`/masih berjalan — restore verification makan waktu 30-70 detik, lebih lama dari backup), menghasilkan entri log baru nyata: `restore_database=epelara_restore_verify_1787992942079_6c46ac47`, `started_at=2026-08-29T08:42:21Z`, outcome **`RESTORE_VERIFIED`**, `checksum_match: true`, 283/283 tabel, `cleanup_status: SUCCESS`. Kadensi mingguan (bukan bulanan) — melebihi syarat minimum Project Owner ("misal tiap bulan"), bukan pelanggaran. Syarat berkelanjutan AIR-009 kini **sepenuhnya operasional**, bukan lagi bergantung eksekusi manual. | Kutipan keputusan Project Owner tercatat di kolom sebelumnya; AIR-EA-001 v1.0.17; verifikasi independen log/`Get-ScheduledTaskInfo` pada sesi ini. |
| Penjadwalan otomatis berkala (RPO 24 jam) | **Operasional, terverifikasi (Addendum kedua, 2026-08-29).** Project Owner mendaftarkan Windows Scheduled Task ("ePeLARA - Database Backup", trigger harian 02:00, `LogonType=S4U`, `DisallowStartIfOnBatteries=False`). Terverifikasi independen: dipicu manual via `Start-ScheduledTask`, menghasilkan backup nyata baru (`backup_id=bkp_1787988620872_6f3547ee`, 88.788.053 bytes, checksum tercatat, file `.sql`+`.manifest.json` dikonfirmasi ada di disk, timestamp cocok). Sempat dua kali gagal senyap (exit 0 tapi tidak melakukan apa pun) sebelum diagnosis menemukan akar masalah: pertama `LogonType=Interactive` tanpa password, kedua `DisallowStartIfOnBatteries=True` (default) menolak jalan saat laptop pakai baterai — keduanya diperbaiki. | Selesai. |
...
| 0.3.0 | 2026-08-29 | **Addendum kedua**: penjadwalan otomatis berkala kini **operasional dan terverifikasi** — Project Owner mendaftarkan Windows Scheduled Task, sempat dua kali gagal senyap (LogonType Interactive, lalu DisallowStartIfOnBatteries default) sebelum diagnosis root-cause dan perbaikan berhasil (verifikasi independen: backup_id baru, file+manifest dikonfirmasi ada di disk). §5/§9 diperbarui. RPO 24 jam kini fully operational, bukan hanya capable. Restore test berkala (bukan hanya backup) tetap belum dijadwalkan — di luar scope addendum ini. | Claude (mode `/loop`, sesi eksekusi backlog), berdasarkan hasil registrasi Project Owner | Approved (evidence addendum) |
| 0.4.0 | 2026-08-29 | **Closure AIR-009**: Project Owner memberikan keputusan closure eksplisit — bukti backup+restore diterima cukup, dengan syarat restore test wajib diulang berkala (rekomendasi bulanan). AIR-009 Resolved → **Closed** di AIR-EA-001 v1.0.17. §9 baris restore test diperbarui mencatat keputusan ini. Penjadwalan otomatis untuk restore test berkala direkomendasikan (pola sama dengan task backup) tapi belum didaftarkan. | Claude, berdasarkan keputusan closure eksplisit Project Owner | Approved |
| 0.5.0 | 2026-08-29 | **Syarat berkelanjutan AIR-009 kini operasional**: Project Owner mendaftarkan task Windows Scheduler kedua ("ePeLARA - Restore Verify", mingguan) menerapkan langsung pelajaran dari task backup (S4U + AllowStartIfOnBatteries sejak awal, tanpa kegagalan senyap). Diverifikasi independen: dipicu manual, ditunggu sampai selesai (bukan asumsi dari status sementara), entri log baru nyata dikonfirmasi (`RESTORE_VERIFIED`, checksum match, 283/283 tabel). §9 diperbarui. | Claude, berdasarkan hasil registrasi Project Owner, verifikasi independen | Approved |

## 14. Validation Checklist (Version 0.5.0)
...
Version 0.5.0, status **Approved**, effective_date 2026-08-06, last_reviewed 2026-08-29. Backup **dan** restore verification kini **keduanya** dijadwalkan otomatis dan terverifikasi operasional (backup harian, restore verify mingguan) — RPO 24 jam fully operational, syarat restore test berkala AIR-009 juga fully operational, bukan lagi bergantung eksekusi manual. **AIR-009 Closed** (2026-08-29).
