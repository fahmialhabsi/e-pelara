...
- **Koreksi 2026-08-29 (setelah eksekusi item ini)**: rencana awal di atas — membuat draft/rancangan "Candidate Target Integration Pattern (API-based)" — **TIDAK dilaksanakan**. Peninjauan ulang ADR-0004 §3 butir 4 dan BP-INT-001 §7 saat mengerjakan item ini menemukan larangan eksplisit "tidak boleh diisi berdasarkan asumsi" untuk Bagian B, lebih ketat dari perkiraan saat runbook ini ditulis. Mengarang skema API untuk SIPD (sistem eksternal spesifik milik Kemendagri yang karakteristiknya tidak diketahui) dinilai melanggar batasan itu. **Yang dikerjakan sebagai gantinya**: dokumentasi kontrak teknis Interim Pattern yang sudah ada dan berjalan nyata (Bagian A) — lihat BP-INT-001 v0.1.1 §6a dan ECHG-091.
...
- **Koreksi 2026-08-29 (setelah eksekusi item ini)**: butir (b) di atas **TIDAK dilaksanakan**. Dokumen REF-EA-001 sendiri (§2, §5.1, §12) eksplisit melarang "menciptakan relationship yang tidak tercatat pada sumber" — populasi detail per-requirement untuk 75 artefak akan memerlukan menyimpulkan banyak relationship spesifik di luar yang tercatat di Master Roadmap §6, melanggar batasan itu. Pola sama seperti koreksi Item 6 (AIR-007/SIPD): rencana awal runbook ditulis sebelum membaca detail batasan dokumen sasaran. Yang dikerjakan: butir (a) — koreksi staleness — dan (c). Lihat REF-EA-001 v1.1.0 §7a dan ECHG-092.
...
|----|------|--------|---------|----------|------------------|
| 1 | AIR-009 Backup & Restore | Selesai | 2026-08-29 | ECHG-086, ECHG-094 | Engine sudah Implemented sejak Sprint 2 (commit 949a3e4f). Backup+restore test nyata: RESTORE_VERIFIED, checksum match, 283/283 tabel. **Update**: Task Scheduler kini terdaftar & terverifikasi operasional (Owner) — sempat 2x gagal senyap (LogonType, lalu DisallowStartIfOnBatteries), didiagnosis dan diperbaiki. RPO 24 jam fully operational. AIR-009 tetap Resolved (bukan Closed) — closure tetap wewenang Owner. |
...
| 4 | AIR-001 Transition Year Renstra | Selesai (verifikasi) | 2026-08-29 | ECHG-089 | Field target_tahun_6/pagu_tahun_6 konsisten di 10 model Renstra; period_type dikonfirmasi belum ada (sesuai Evidence Pending). BP-DATA-003/GOV-DATA-001 dikonfirmasi sudah benar merutekan aturan pemicu ke institutional/statutory authority. Aturan pemicu itu sendiri di luar wewenang loop (ADR-0001 §3.1) — sengaja tidak dibuat candidate rule (beda dari SIPD, ini pertanyaan legal bukan teknis). |
| 5 | AIR-004 Workflow / 32a | Selesai | 2026-08-29 | ECHG-090 | RPJMD sync + admin unification (dua item ADR-0002) sudah diperbaiki di kode (uncommitted), diverifikasi (8/8, 18/18 pass) dan dikomit sesi ini. 32a Tahap 1/Tahap 3 Opsi C diverifikasi independen (dijalankan nyata). Status keseluruhan 32a tetap Sebagian Diimplementasikan (Tahap 2 tetap N/A, tidak dipaksakan). |
