...
export default function useSubKegiatanFormLogic(existingData, onSubmit) {
  ...
  const [selectedMasterSubKegiatanId, setSelectedMasterSubKegiatanId] = useState('');
  ...
  useEffect(() => {
    if (existingData && programList.length > 0 && dokumen && tahun) {
      ...
      setTotalPaguProgram(existingData.kegiatan?.program?.total_pagu_anggaran ?? 0);
  ...
  const selectedRpjmProgram = useMemo(
    () => programList.find((p) => String(p.id) === String(formData.program_id)) || null,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await fetchMasterKegiatanByProgram(masterProgramIdForKegiatanFetch, {
  ...
  const selectedRpjmKegiatan = useMemo(
    () => kegiatanList.find((k) => String(k.id) === String(formData.kegiatan_id)) || null,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await fetchMasterSubKegiatanByKegiatan(masterKegiatanIdForSubFetch, {
          datasetKey: referensiDatasetKey,
        });
        if (!cancelled) {
  ...
  const selectedMasterSubOption = useMemo(() => {
    if (masterPathActive && selectedMasterSubKegiatanId) {
      return (
        masterSubKegiatanOptions.find((o) => o.value === String(selectedMasterSubKegiatanId)) ||
        null
      );
    }
  ...
  const applyMasterSubSelection = (opt) => {
    ...
    setFormData((prev) => ({
      ...
      master_sub_kegiatan_id: opt.masterSubId != null ? String(opt.masterSubId) : '',
      regulasi_versi_id: vid != null && vid !== '' ? String(vid) : '',
    }));
    setSelectedMasterSubKegiatanId(
      masterPathActive && opt.masterSubId != null ? String(opt.masterSubId) : '',
    );
  };

  const handleProgramChange = async (programId, prefillKegiatanId = '', opts = {}) => {
  ...
  const handleMasterKegiatanChange = (e) => {
    ...
    if (!rk) {
      setMessage('Kegiatan referensi tidak memiliki pasangan di kegiatan RPJMD untuk program ini.');
      setFormData((prev) => ({
        ...prev,
        kegiatan_id: '',
  ...
  useEffect(() => {
    ...
    const effectiveProgramId =
      existingData.program_id ?? existingData.kegiatan?.program_id ?? formData.program_id ?? '';

    const directMasterProgramId =
      existingData.master_program_id ?? existingData.kegiatan?.program?.master_program_id ?? null;
    ...
    const programTransaksi = programList.find((p) => String(p.id) === String(effectiveProgramId));
    ...
    masterEditHydrateRef.current.programId = existingData.id;
  }, [existingData, masterProgramList, programList, formData.program_id]);
  ...
  useEffect(() => {
    ...
    const effectiveKegiatanId = existingData.kegiatan_id ?? formData.kegiatan_id ?? '';

    const directMasterKegiatanId =
      existingData.master_kegiatan_id ?? existingData.kegiatan?.master_kegiatan_id ?? null;
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      formData.pagu_anggaran == null ||
      isNaN(rawPagu) ||
      rawPagu < 0
    ) {
      setMessage('❌ Pagu Anggaran wajib diisi dengan angka valid (boleh 0).');
      setChecking(false);
      return;
    }

    if (rawPagu > 1_000_000_000_000) {
      setMessage('❌ Pagu Anggaran terlalu besar. Maksimal 1 Triliun untuk saat ini.');
      setChecking(false);
      return;
    }

    const submitAsMaster = masterPathActive && normStr(formData.master_sub_kegiatan_id);
    ...
    if (submitAsMaster) {
      ...
      if (!normStr(formData.master_sub_kegiatan_id)) {
        setMessage('❌ Mode MASTER: pilih sub kegiatan dari referensi master terlebih dahulu.');
        setChecking(false);
        return;
      }
    ...
    try {
      ...
      setMessage('❌ Terjadi kesalahan saat menyimpan. Silakan cek kembali input Anda.');
    } finally {
      setChecking(false);
    }
  };
  ...
  return {
    ...
    totalPaguProgram,
    masterSubKegiatanOptions,
    masterSubLoading: masterProgramsLoading || masterKegiatanLoading || masterSubLoading,
