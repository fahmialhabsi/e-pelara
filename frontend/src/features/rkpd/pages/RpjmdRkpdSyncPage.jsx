...
export function syncCategoryMeta(category) {
  ...
  const meta = {
    ...
    source_unmapped: {
      ...
      action: 'Jalankan Auto Mapping Program/Kegiatan/Sub atau backfill, lalu preview ulang.',
    },
...
export default function RpjmdRkpdSyncPage() {
  ...
  const [srcTahun, setSrcTahun] = useState(String(dokumenTahun || new Date().getFullYear()));
  const [tgtPeriodeId, setTgtPeriodeId] = useState('');
  const [tgtTahun, setTgtTahun] = useState(String(dokumenTahun || new Date().getFullYear()));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        rows.sort((a, b) =>
          String(a.kode_sub_kegiatan || '').localeCompare(String(b.kode_sub_kegiatan || ''), 'id'),
        );
  ...
  const masterOptions = useMemo(() => {
    ...
    if (sp && Number.isInteger(selN) && selN >= 1 && !m.has(selN)) {
      m.set(selN, {
        id: selN,
  ...
  const selectedProgram = useMemo(
    () => (selProgramId ? programOptions.find((p) => String(p.id) === String(selProgramId)) : null),
    [programOptions, selProgramId],
  );
  ...
  const buildPayload = useCallback(() => {
    ...
    const body = {
      ...
      filters: {
        program_ids: selProgramId ? [parseInt(selProgramId, 10)] : parseIdList(filterProgram),
        kegiatan_ids: selKegiatanId ? [parseInt(selKegiatanId, 10)] : parseIdList(filterKegiatan),
        sub_kegiatan_ids: selSubId ? [parseInt(selSubId, 10)] : parseIdList(filterSub),
  ...
  const previewOk = Boolean(previewData?.summary) && !previewBlocked && !previewLoading && lastBody;
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      setMessage(e?.response?.data?.message || e?.message || 'Preview sync gagal.');
    } finally {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const extra = d?.commit_blocked_reasons?.messages?.length
        ? ` ${d.commit_blocked_reasons.messages.join('; ')}`
        : '';
      setMessage((e?.response?.data?.message || e?.message || 'Commit gagal.') + extra);
    } finally {
  ...
  const handleAutoMapPreview = async (level) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        patchAutoMapLevel(level, {
          message: res.data?.message || `Pratinjau auto mapping ${cfg.title.toLowerCase()} gagal.`,
        });
      }
    } catch (e) {
      patchAutoMapLevel(level, {
  ...
  const handleAutoMapExecute = async (level) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        patchAutoMapLevel(level, {
          message: res.data?.message || `Eksekusi auto mapping ${cfg.title.toLowerCase()} ditolak.`,
        });
      }
    } catch (e) {
      patchAutoMapLevel(level, {
  ...
  return (
    <Container className="my-4">
      ...
      <p className="text-muted small">
        ...
        <Badge bg="secondary">SUPER_ADMIN</Badge> / <Badge bg="secondary">ADMINISTRATOR</Badge>.
        Tidak ada overwrite baris RKPD existing; insert hanya jika slot unik (perhatikan constraint
        periode+kode sub). Nonaktifkan fitur dengan env <code>RPJMD_RKPD_SYNC_ENABLED=0</code>.
      </p>
      <Alert variant="light" className="small border py-2 mb-3">
        ...
        <em>Program</em>, lalu <em>Kegiatan</em>, lalu <em>Sub kegiatan</em>. Kosongkan semua filter
        untuk meninjau seluruh RPJMD pada periode/tahun tersebut. Pilihan dropdown menggantikan
        kolom ID teks kecuali Anda membuka mode lanjutan.
      </Alert>
      ...
      <Card className="mb-3 border-info shadow-sm">
        <Card.Body>
          ...
          <ol className="small mb-2 ps-3">
            ...
            <li>
              Jika masih ada data sumber belum termapping, jalankan Auto Mapping Sub Kegiatan.
            </li>
          ...
          <Alert variant="light" className="small py-2 mb-0">
            Sync hanya menambahkan data yang aman dan tidak menimpa data RKPD existing. Pratinjau
          ...
          <Card className="mb-3 shadow-sm">
            <Card.Body>
              ...
              <Form.Group className="mb-2">
                ...
                <Form.Select value={srcPeriodeId} onChange={(e) => setSrcPeriodeId(e.target.value)}>
              ...
              <Form.Group className="mb-2">
                ...
                <Form.Select value={tgtPeriodeId} onChange={(e) => setTgtPeriodeId(e.target.value)}>
                  <option value="">— pilih periode —</option>
                  {periodeList.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.nama} ({p.tahun_awal}–{p.tahun_akhir}) · id {p.id}
              ...
              <Form.Group className="mb-2">
                ...
                >
                  <option value="">
                    {selKegiatanId ? '— semua sub pada kegiatan —' : '— pilih kegiatan dulu —'}
                  </option>
                  {subOptions.map((s) => (
                    <option key={s.id} value={String(s.id)}>
                      {s.kode_sub_kegiatan} {labelTruncate(s.nama_sub_kegiatan, 48)} · id {s.id}
                    </option>
                  ))}
                </Form.Select>
              </Form.Group>
                ...
                {selectedProgram ? (
                  selectedMasterId != null ? (
                    <div className="small mb-2 p-2 bg-light rounded border">
                      Program terpilih punya <code>master_program_id = {selectedMasterId}</code> di
                      data RPJMD. Filter ini <strong>tidak wajib</strong> — memilih Program saja
                      sudah membatasi sync. Gunakan master hanya bila ingin membatasi tambahan ke
                      baris dengan master tertentu.
                      <div className="mt-2">
                        <Button
                          type="button"
                    ...
                    <div className="small mb-2 p-2 bg-warning bg-opacity-10 rounded border border-warning">
                      ...
                      <code>master_program_id</code> (masih kosong di database). Dropdown di bawah
                      hanya berisi master dari program lain di daftar OPD Anda — sering kali hanya
                      opsi &quot;tidak filter&quot;. Itu <strong>normal</strong> untuk data LEGACY /
                      belum di-link ke master.
                      <div className="mt-1">
                      ...
                      <div className="mt-2 small">
                        ...
                        <strong>backfill mapping</strong> (preview → execute) atau impor dari master
                        referensi — misalnya lewat{' '}
              ...
              >
                {showAdvancedIds ? 'Sembunyikan' : 'Tampilkan'} input ID teks (banyak ID /
                copy-paste)
              </Button>
            ...
            <Card.Body>
              ...
              <Row className="g-2">
                {Object.entries(AUTO_MAP_LEVEL_CONFIG).map(([level, cfg]) => {
                  ...
                  const massScan =
                    !Array.isArray(payloadLv?.[cfg.idField]) || payloadLv[cfg.idField].length < 1;
                    ...
                    <Col xl={4} md={12} key={level}>
                      <div className="border rounded p-2 h-100 bg-light">
                        ...
                        <Form.Check
                          ...
                          onChange={(e) => patchAutoMapLevel(level, { confirm: e.target.checked })}
              ...
              <Card.Body>
                ...
                {previewData.correlation_id ? (
                  <p className="small text-muted">
                    correlation_id: <code>{previewData.correlation_id}</code>
                  </p>
                ) : null}
                {previewBlocked ? (
                  <Alert variant="danger" className="small">
                    <strong>Commit belum dapat dilanjutkan.</strong> Pratinjau menemukan
                    konflik/validasi yang harus diselesaikan terlebih dahulu.
                  </Alert>
                ) : (
                ...
                <Form.Group className="mb-2">
                  <Form.Label className="small">Alasan commit (opsional, audit)</Form.Label>
                  <Form.Control
                    as="textarea"
                    rows={2}
                    value={commitReason}
                ...
                <Form.Text className="d-block mt-2 text-muted">
                  Commit menjalankan preflight yang sama dengan preview. Payload terakhir disimpan
                  dari preview sukses.
                </Form.Text>
              </Card.Body>
            </Card>
