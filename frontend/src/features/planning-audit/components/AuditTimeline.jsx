 ...
 */
export default function AuditTimeline({ rows = [], loading, fieldLabelMap = {} }) {
  if (loading) {
  ...
  return (
    <ul className="space-y-3 border-l-2 border-gray-200 pl-4">
      {rows.map((ev) => {
        ...
        const when = ev.changed_at ? new Date(ev.changed_at).toLocaleString('id-ID') : '—';
        return (
          <li key={ev.id} className="relative text-sm">
            ...
            {n.summary ? (
              <div className="mt-1 rounded bg-slate-50 px-2 py-1 text-xs text-gray-800">
                {n.summary}
              </div>
            ) : null}
            {Array.isArray(n.changed_fields) && n.changed_fields.length > 0 ? (
              <ul className="mt-1 list-inside list-disc text-xs text-gray-700">
                {n.changed_fields.slice(0, 8).map((c) => (
                  <li key={mapKey(c.field)}>
                    <span className="font-medium">{mapKey(c.field)}</span>: {String(c.from ?? '—')}{' '}
                    → {String(c.to ?? '—')}
                  </li>
                ))}
              </ul>
