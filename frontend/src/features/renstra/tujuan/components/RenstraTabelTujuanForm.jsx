...
const RenstraTabelTujuanForm = ({ initialData = null, renstraAktif }) => {
  ...
  const { data: tabelTujuanByTujuan = [], isLoading: loadingTabelTujuan } = useQuery({
    queryKey: ['renstra-tabel-tujuan-by-tujuan', tujuanId],
    queryFn: async () => {
      const res = await api.get(`/renstra-tabel-tujuan/by-tujuan/${tujuanId}`);
      return res.data;
    },
    enabled: !!tujuanId,
  });
  ...
  const generatePayload = (data) => {
    const selectedTujuan = tujuanOptions.find((t) => String(t.id) === String(data.tujuan_id));
    ...
    return {
      ...
      misi_id: selectedTujuan?.misi_id ?? null,
  ...
  const { data: indikatorOptions = [], isLoading: loadingIndikator } = useQuery({
    queryKey: ['indikator-renstra', renstraAktif?.id, 'tujuan', selectedTujuanId],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    const selected = tujuanOptions.find((t) => String(t.id) === String(selectedTujuanId));
  ...
  const breadcrumbChain = useRenstraBreadcrumb({
    currentLabel: initialData ? 'Edit Tujuan' : 'Tambah Tujuan',
  });
  ...
  const handleSubmitRevisi = async (data) => {
    ...
    try {
      ...
      navigate('/renstra/tabel/program');
    } catch (err) {
      setServerMessage(
  ...
  if (!renstraAktif || loadingIndikator || loadingTujuan || loadingTabelTujuan) {
    return <SpinnerFullscreen tip="Memuat data..." />;
  }

  return (
    <Card title={initialData ? 'Edit Renstra Tabel Tujuan' : 'Tambah Renstra Tabel Tujuan'}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <ul>
            {tabelTujuanByTujuan.map((row) => (
              <li key={row.id}>
                {row.kode_tujuan} - {row.nama_tujuan} | {row.indikator?.nama_indikator}
              </li>
            ))}
          </ul>
        </div>
          ...
          <div style={{ marginTop: 32 }}>
            ...
            {historyRows.map((row) => (
              ...
              >
                ...
                <p>
                  <strong>Target Sebelum:</strong> {row.before_json?.target_akhir_renstra ?? '-'}
                </p>

                <p>
                  <strong>Target Setelah:</strong> {row.after_json?.target_akhir_renstra ?? '-'}
                </p>
