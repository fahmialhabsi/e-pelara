...
export default function RenstraTabelArahKebijakanForm({ initialData = null, renstraAktif }) {
  ...
  const defaultValues = {
    ...
    kebijakan_id: initialData?.kebijakan_id
      ? String(initialData.kebijakan_id)
      : "",
  ...
  const paguAkhir = paguValues.reduce((a, b) => a + b, 0);

  return {
    renstra_id: Number(renstraId),
    strategi_id: selectedStrategi ? Number(selectedStrategi.id) : null,
    kebijakan_id: selectedKebijakan ? Number(selectedKebijakan.id) : Number(data.kebijakan_id),
    ...
    target_akhir_renstra: Number(targetAkhir.toFixed(2)),
    pagu_akhir_renstra: paguAkhir,
  ...
  const editStrategiId =
  initialData?.kebijakan?.strategi_id || initialData?.strategi_id;
  ...
  queryKey: ["indikator-renstra", renstraId, "kebijakan", selectedKebijakanId],
  queryFn: async () => {
    const res = await api.get("/indikator-renstra", {
      params: {
        renstra_id: renstraId,
        stage: "kebijakan",
        ref_id: selectedKebijakanId,
      },
    });
    ...
    enabled: !!renstraId && !!selectedKebijakanId,
  });
  ...
  useEffect(() => {
    ...
    const isEditStrategi =
      initialData &&
      String(selectedStrategiId) === String(editStrategiId);

    const source = isEditStrategi
      ? strategiFromEdit || selectedStrategi
      : selectedStrategi;
  ...
  useEffect(() => {
    ...
    if (selectedKebijakan) {
      setValue("kode_kebijakan", selectedKebijakan.kode_kebjkn || selectedKebijakan.kode_kebijakan || selectedKebijakan.kode || "");
      setValue("deskripsi_kebijakan", selectedKebijakan.deskripsi || selectedKebijakan.isi_kebijakan || "");
    }
  }, [selectedKebijakanId, kebijakanOptions, setValue, initialData, renstraId]);
  ...
  const editLabel = makeLabel(
    strategiFromEdit?.kode_strategi ||
      strategiFromEdit?.kode ||
      initialData?.kode_strategi,
...
const kebijakanSelectOptions = useMemo(() => {
  const options = kebijakanOptions.map((item) => ({
  ...
  const indikatorSelectOptions = useMemo(() => {
    ...
    setValue(
      "kebijakan_id",
      initialData?.kebijakan_id ? String(initialData.kebijakan_id) : ""
    );
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setValue(
        "pagu_akhir_renstra",
        YEARS.reduce(
          (total, i) => total + Number(initialData[`pagu_tahun_${i}`] || 0),
          0
        ),
        {
          shouldValidate: false,
    ...
    setPaguInfoMessage(
      totalAcuan
        ? "Pagu otomatis dari acuan RPJMD/cache. Pagu dapat direvisi setelah edit."
        : "Pagu akan muncul otomatis setelah cache tersinkron."
    );
    ...
    setExistingDataInfo(null);
      ...
      try {
        ...
        if (isAuditMode) {
      setServerMessage(
        "Data approved dalam mode audit. Klik Buat Revisi untuk mengubah."
      );
      return;
    }
    ...
    const isLoading =
      !renstraAktif || loadingStrategi || loadingKebijakan || loadingPaguCache;
    ...
    <Card title={formTitle}>
      ...
      {isLoading ? <div>Memuat data...</div> : (
        <form onSubmit={handleSubmit(initialData ? handleSubmitRevisi : onSubmit)}>
          ...
          />
          <InputField name="kode_kebijakan" label="Kode Arah Kebijakan" control={control} errors={errors} disabled />
          <InputField name="deskripsi_kebijakan" label="Deskripsi Arah Kebijakan" control={control} errors={errors} disabled />
          ...
          <CurrencyInputField name="pagu_akhir_renstra" label="Pagu Akhir Renstra" control={control} errors={errors} disabled />
            ...
            {!existingDataInfo || initialData ? (
              isAuditMode ? (
                <Button
                  ...
                  onClick={() =>
                    navigate(
                      `/renstra/tabel/arah-kebijakan/edit/${initialData.id}?mode=revisi`
                    )
                  }
                >
                  Buat Revisi
