...
const QUERY_KEY = "renstra-tabel-arah-kebijakan";
...
export default function RenstraTabelArahKebijakanListPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      message.success("Data arah kebijakan berhasil dihapus");
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
    },
    onError: (error) => {
      message.error(
        error?.response?.data?.message ||
          "Gagal menghapus data arah kebijakan"
      );
    },
  });
  ...
  const columns = [
    {
      title: "Kode",
      dataIndex: "kode_kebijakan",
      key: "kode_kebijakan",
      width: 140,
      fixed: "left",
      ellipsis: true,
      render: (value, record) =>
        value || record.kebijakan?.kode_kebjkn || "-",
    },
    {
      ...
      render: (_, record) => (
        <div style={wrapTextStyle}>
          {record.deskripsi_kebijakan || record.kebijakan?.deskripsi || "-"}
        </div>
      ),
    },
    {
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {!isApproved && (
              <Button
                ...
                type="primary"
                onClick={() =>
                  navigate(`/renstra/tabel/arah-kebijakan/edit/${record.id}`)
                }
              >
                Edit Draft
              </Button>
            ...
            {isApproved && (
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/renstra/tabel/arah-kebijakan/edit/${record.id}?mode=revisi`
                  )
                }
              >
                Buat Revisi
            ...
            <Button
              size="small"
              onClick={() =>
                navigate(`/renstra/tabel/arah-kebijakan/history/${record.id}`)
              }
            >
              History
            </Button>
            ...
            {!isApproved && (
              <Popconfirm
                title="Hapus data arah kebijakan ini?"
  ...
  return (
    <Card title="Renstra Tabel Arah Kebijakan">
      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
      ...
      >
        ...
        <Button
          type="primary"
          onClick={() => navigate("/renstra/tabel/arah-kebijakan/add")}
        >
          Tambah
        </Button>
                ...
                extraMeta={[
                  { label: "Lokasi", value: record.lokasi },
                  {
                    ...
                    value:
                      ...
                      record.bidang_penanggung_jawab ||
                      record.kebijakan?.strategi?.renstra?.bidang_opd ||
