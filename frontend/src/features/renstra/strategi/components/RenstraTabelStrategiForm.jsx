...
export default function RenstraTabelStrategiForm({ initialData = null, renstraAktif }) {
  ...
  strategi_id: initialData?.strategi_id
  ...
  const selectedStrategi = strategiOptions.find(
    (x) => String(x.id) === String(selectedStrategiId)
  );
  
  const { data: indikatorOptions = [], isLoading: loadingIndikator } = useQuery({
    queryKey: ["indikator-renstra", renstraId, "strategi", selectedStrategiId],
    queryFn: async () => {
      const res = await api.get("/indikator-renstra", {
        params: {
          renstra_id: renstraId,
          stage: "strategi",
          ref_id: selectedStrategiId,
        },
      });
    ...
    enabled: !!renstraId && !!selectedStrategiId,
  });
...
useEffect(() => {
  ...
  const selected = strategiOptions.find(
    (s) => String(s.id) === String(selectedStrategiId)
  );
...
useEffect(() => {
    ...
    if (!initialData) {
      ...
      setPaguInfoMessage(
        `Pagu RPJMD Rp ${paguAcuan.toLocaleString(
          "id-ID"
        )} dibagi ke tahun 1–5. Jika ada sisa, masuk tahun ke-5.`
      );
    }

}, [selectedIndikatorId, indikatorOptions.length, setValue]);
  ...
  const paguValues = useWatch({
    ...
    const paguValuesKey = paguValues.join("|");
  ...
  !renstraAktif ||
  loadingStrategi ||
  (!!selectedStrategiId && loadingIndikator);
    ...
    <Card title={initialData ? "Edit Tabel Strategi" : "Tambah Tabel Strategi"}>
      ...
      {isLoading ? <div>Memuat data...</div> : (
        <form onSubmit={handleSubmit(initialData ? handleSubmitRevisi : onSubmit)}>
          ...
          {YEARS.map((i) => <InputField key={`t${i}`} name={`target_tahun_${i}`} label={`Target (th. ke-${i})`} control={control} errors={errors} />)}
