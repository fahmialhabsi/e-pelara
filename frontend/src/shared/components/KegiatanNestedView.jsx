...
const formatRupiah = (value) => `Rp ${toNumber(value).toLocaleString('id-ID')}`;
...
const sumKegiatanList = (kegiatanList) => {
  return (kegiatanList || []).reduce((total, keg) => total + getKegiatanPagu(keg), 0);
};
    ...
    <>
      <div className="mb-3 p-3 border rounded bg-light">
        <div className="fw-bold text-primary">
          Total Pagu RPJMD:{' '}
          <span className="badge bg-info text-dark">{formatRupiah(totalPaguOpd)}</span>
        </div>
      </div>
          ...
          <Accordion.Item eventKey={i.toString()} key={tujuan}>
            <Accordion.Header>
              <div className="d-flex flex-column">
                <span style={{ fontWeight: 'bold', color: '#0f172a' }}>{tujuan}</span>
            ...
            <Accordion.Body>
              <Accordion alwaysOpen>
                {Object.entries(sasaranMap).map(([sasaran, programMap], j) => (
                  <Accordion.Item eventKey={`s-${i}-${j}`} key={`${tujuan}-${sasaran}`}>
                    <Accordion.Header>
                      <div className="d-flex flex-column">
                        <span style={{ fontWeight: 'bold', color: '#1e3a8a' }}>{sasaran}</span>
                        <small className="text-muted">
                    ...
                    <Accordion.Body>
                      <Accordion alwaysOpen>
                        {Object.entries(programMap).map(([program, kegiatanList], k) => {
                          const firstKegiatan = kegiatanList?.[0];
                          const totalPaguProgram = sumKegiatanList(kegiatanList);
                            ...
                            >
                              <Accordion.Header>
                                <div className="d-flex flex-column">
                                  ...
                                  <small className="text-muted">
                                    Total Pagu Program: {formatRupiah(totalPaguProgram)}
                                  </small>
                                </div>
                              </Accordion.Header>
                                  ...
                                  {kegiatanList.map((keg) => (
                                    ...
                                    >
                                      <div>
                                        ...
                                        >
                                          {keg.kode_kegiatan} - {capitalize(keg.nama_kegiatan)}
                                        </span>
                                        ...
                                        <div className="mt-2 text-muted small">
                                          ...
                                          <span className="badge bg-info text-dark">
                                            {formatRupiah(
                                              keg.total_pagu_anggaran ?? keg.pagu_anggaran,
                                            )}
                                          </span>
                                        </div>
                                        ...
                                        <div className="mt-2 text-muted small">
                                          ...
                                          <span className="badge bg-secondary">
                                            {keg.program?.bidang_opd_penanggung_jawab ||
