...
export const sanitizeIndikator = (item) => {
  const sanitizedItem = {
    ...
    penanggung_jawab:
      item.penanggung_jawab != null && item.penanggung_jawab !== ""
        ? String(item.penanggung_jawab)
        : "",
    keterangan: item.keterangan ?? "",
    satuan: item.satuan,
    misi_id:
      item.misi_id != null && String(item.misi_id).trim() !== ""
        ? Number(item.misi_id)
        : null,
    tujuan_id:
      item.tujuan_id != null && String(item.tujuan_id).trim() !== ""
        ? Number(item.tujuan_id)
        : null,
    periode_id:
      item.periode_id != null && item.periode_id !== ""
