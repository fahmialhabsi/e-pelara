...
const getReportQualityGate = (report = {}) => {
  return (
    report.report_quality_gate || report.quality_gate || report.final_report_quality_gate || {}
  );
};
...
const getPdfExportGate = (report = {}) => {
  ...
  return {
    is_final_pdf: readyToSign && pdfReady && finalReportStatus === PDF_READY_STATUS,
    is_draft_pdf: !readyToSign || !pdfReady || finalReportStatus !== PDF_READY_STATUS,
    document_status_label:
      approvalGate.document_status_label || 'DRAFT — BELUM SIAP DITANDATANGANI',
    warning:
...
const buildPdfFromWord = async (contextId, options = {}) => {
