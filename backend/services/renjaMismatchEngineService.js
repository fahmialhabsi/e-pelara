...
const RENJA_ITEM_RULES = [
  {
    ...
    description: 'Item RENJA tidak punya padanan RENSTRA',
    severity: 'warning',
    is_blocking: false,
    scope: 'item',
    source_type: 'RENSTRA',
  },
  {
    ...
    description: 'Item RENJA tidak punya padanan RKPD',
    severity: 'warning',
    is_blocking: false,
    scope: 'item',
    source_type: 'RKPD',
  },
  {
    ...
    description: 'Relasi parent-child item tidak valid',
    severity: 'error',
    is_blocking: true,
    scope: 'item',
  ...
  {
    code: 'CODE_CHANGED',
    description: 'Kode item berubah dari sumber',
    severity: 'warning',
    is_blocking: false,
    scope: 'item',
    source_type: 'RENSTRA',
  ...
  {
    code: 'NAME_CHANGED',
    description: 'Nama item berubah dari sumber',
    severity: ' warning',
    is_blocking: false,
    scope: 'item',
    source_type: 'RENSTRA',
  },
  {
    ...
    description: 'Item memakai source mode MANUAL',
    severity: 'info',
    is_blocking: false,
    scope: 'item',
    source_type: 'INTERNAL',
...
async function computeMismatchForDokumen(db, renjaDokumenId, opts = {}) {
  ...
  const chainEntries = await Promise.all(
    items.map(async (it) => {
      const bySql = await policySvc
        .getPolicyChainByRenjaItem(db, it.id, { transaction: tx })
        .catch(() => null);
      const chain =
        bySql ||
        (await policySvc
          .getPolicyChainByRenjaItemSequelize(db, it.id, { transaction: tx })
          .catch(() => null));
  ...
  for (const x of crossDocResults) {
    const item = x.related_item_id
      ? items.find((it) => Number(it.id) === Number(x.related_item_id))
      : null;
  ...
  for (const item of items) {
    ...
    const hierarchyIndicators = chain
      ? await policySvc.getIndicatorsAcrossHierarchy(db, chain).catch(() => null)
      : null;
    ...
    if (item.source_renstra_subkegiatan_id) {
      ...
      if (renstraSub) {
        if (
          ...
          renstraSub.kode_sub_kegiatan &&
          String(item.kode_sub_kegiatan) !== String(renstraSub.kode_sub_kegiatan)
        ) {
          results.push(
            mk({
              renjaDokumenId,
    ...
    const rkpdItem = item.source_rkpd_item_id
      ? rkpdById.get(Number(item.source_rkpd_item_id))
      : null;
    ...
    if (item.source_indikator_renstra_id) {
      ...
      renstraIndicator = ir
        ? { id: ir.id, nama: ir.nama_indikator }
        : { id: item.source_indikator_renstra_id, nama: null };
    }
    ...
    const expectedIndicators = hierarchyIndicators?.expected
      ? Object.values(hierarchyIndicators.expected)
          .flat()
          ...
          if (bestSim < cfg.indicator_similarity_warning_threshold) {
            results.push(
              mk({
                ...
                recommendation:
                  'Samakan indikator dengan referensi sumber atau beri justifikasi manual override.',
                renjaItem: item,
                fieldName: 'indikator',
...
async function persistMismatchResults(
  db,
  renjaDokumenId,
  engineOutput,
  runType = 'full',
