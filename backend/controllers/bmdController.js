const { Bmd, PeriodeRpjmd } = require("../models");
const Joi = require("joi");

// Validasi input
const bmdSchema = Joi.object({
...
module.exports = {
  ...
  async getById(req, res) {
    try {
      const { id } = req.params;
      const data = await Bmd.findByPk(id, {
        include: [{ model: PeriodeRpjmd, as: "periode" }],
      });
  ...
  async create(req, res) {
    try {
      const { error, value } = bmdSchema.validate(req.body);
      if (error)
        return res.status(400).json({ error: error.details[0].message });

      const created = await Bmd.create(value);
  ...
  async update(req, res) {
    try {
      ...
      const [updated] = await Bmd.update(value, { where: { id } });
      if (!updated)
        return res.status(404).json({ error: "Data tidak ditemukan" });

      const result = await Bmd.findByPk(id);
      res.json(result);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  ...
  async destroy(req, res) {
    try {
      const { id } = req.params;
      const deleted = await Bmd.destroy({ where: { id } });
      if (!deleted)
        return res.status(404).json({ error: "Data tidak ditemukan" });
