...
const svc = require("../services/rpjmdProgramAutoMapService");
...
const ACTION = {
  PREVIEW: "RPJMD_PROGRAM_AUTO_MAP_PREVIEW",
  EXECUTE: "RPJMD_PROGRAM_AUTO_MAP_EXECUTE",
  EXECUTE_REJECTED: "RPJMD_PROGRAM_AUTO_MAP_EXECUTE_REJECTED",
};

function digestRequest(body) {
  const payload = body && typeof body === "object" ? body : {};
  const ids = Array.isArray(payload.program_ids) ? payload.program_ids : [];
  return {
    dataset_key: payload.dataset_key || svc.DEFAULT_DATASET_KEY,
    periode_id: payload.periode_id,
    tahun: payload.tahun,
    jenis_dokumen: payload.jenis_dokumen,
    program_ids_count: ids.length,
    program_ids_head: ids.slice(0, 50),
...
function compactExecute(data) {
  ...
  return {
    ...
    mapped_program_ids_head: (data.mapped_program_ids || []).slice(0, 120),
...
function baseAuditPayload({ req, correlation_id, action, reason }) {
  return wrapComplianceBase({
    req,
    change_origin: CHANGE_ORIGIN.API_RPJMD_PROGRAM_AUTO_MAP,
    action,
    entity_type: "PROGRAM_AUTO_MAP",
    entity_id: null,
    entity_scope_override: "BULK:RPJMD_PROGRAM_AUTO_MAP",
    affected_ids: null,
...
async function preview(req, res) {
  ...
  try {
    const result = await svc.previewProgramAutoMap(req.body || {});
    ...
    writeTenantAudit({
      ...
      payload: {
        ...
        scanned_count: result.data?.summary?.total_programs_scanned ?? 0,
        mapped_count: 0,
        skipped_count:
        ...
        result_compact: result.ok
          ...
          : { error: result.error || "Preview auto mapping ditolak." },
      },
    });
    ...
    if (!result.ok) {
      return res.status(400).json({
        success: false,
        message: result.error || "Preview auto mapping program ditolak.",
        data: { correlation_id },
      });
    }
    ...
    console.error("[rpjmdProgramAutoMapController.preview]", err);
    ...
    return res.status(500).json({
      success: false,
      message: "Gagal menjalankan preview auto mapping program.",
      data: { correlation_id },
    });
  }
}
...
async function execute(req, res) {
  ...
  try {
    const result = await svc.executeProgramAutoMap(req.body || {});
    ...
    if (!result.ok) {
      writeTenantAudit({
        ...
        payload: {
          ...
          change_origin: CHANGE_ORIGIN.API_RPJMD_PROGRAM_AUTO_MAP,
          request_digest: digestRequest(req.body),
          scanned_count: result.data?.preview_summary?.total_programs_scanned ?? 0,
          mapped_count: 0,
          skipped_count: 0,
          error: result.error || "Execute auto mapping ditolak.",
        },
      });

      return res.status(400).json({
        success: false,
        message: result.error || "Execute auto mapping program ditolak.",
        data: { correlation_id },
      });
    }
    ...
    writeTenantAudit({
      ...
      payload: {
        ...
        skipped_count:
          result.data?.summary?.already_mapped_count ??
          result.data?.summary?.skipped ??
          0,
        failed_count: result.data?.summary?.failed ?? 0,
        affected_ids: (result.data?.mapped_program_ids || []).slice(0, 200),
    ...
    console.error("[rpjmdProgramAutoMapController.execute]", err);
    ...
    return res.status(500).json({
      success: false,
      message: "Gagal menjalankan execute auto mapping program.",
      data: { correlation_id },
    });
  }
}
