const { Pengkeg, PeriodeRpjmd } = require("../models");
...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const data = await Pengkeg.findAll({
        where,
  ...
  async getById(req, res) {
    try {
      const { id } = req.params;
      const data = await Pengkeg.findByPk(id, {
        include: [{ model: PeriodeRpjmd, as: "periode" }],
      });
  ...
  async create(req, res) {
    try {
      ...
      const created = await Pengkeg.create(value);
  ...
  async update(req, res) {
    try {
      ...
      const [updated] = await Pengkeg.update(value, { where: { id } });
      if (!updated)
        return res.status(404).json({ error: "Data tidak ditemukan" });

      const result = await Pengkeg.findByPk(id);
      res.json(result);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
