const { Dpa, PeriodeRpjmd } = require("../models");
...
module.exports = {
  ...
  async create(req, res) {
    try {
      const { error, value } = dpaSchema.validate(req.body);
      if (error)
        return res.status(400).json({ error: error.details[0].message });

      const created = await Dpa.create(value);
  ...
  async update(req, res) {
    try {
      ...
      const [updated] = await Dpa.update(value, { where: { id } });
      if (!updated)
  ...
  async destroy(req, res) {
    try {
      const { id } = req.params;
      const deleted = await Dpa.destroy({ where: { id } });
      if (!deleted)
        return res.status(404).json({ error: "Data tidak ditemukan" });
