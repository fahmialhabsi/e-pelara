...
const ACTION = {
  PREVIEW: "RPJMD_SUB_AUTO_MAP_PREVIEW",
  EXECUTE: "RPJMD_SUB_AUTO_MAP_EXECUTE",
  EXECUTE_REJECTED: "RPJMD_SUB_AUTO_MAP_EXECUTE_REJECTED",
};

function digestRequest(body) {
  const payload = body && typeof body === "object" ? body : {};
  const ids = Array.isArray(payload.sub_kegiatan_ids)
    ? payload.sub_kegiatan_ids
    : [];
  return {
    ...
    sub_kegiatan_ids_count: ids.length,
    sub_kegiatan_ids_head: ids.slice(0, 50),
...
function compactPreview(data) {
  ...
  return {
    ...
    parent_kegiatan_unmapped_items_sample: (
      data.parent_kegiatan_unmapped_items || []
    ).slice(0, 20),
...
function compactExecute(data) {
  ...
  return {
    ...
    mapped_sub_kegiatan_ids_head: (data.mapped_sub_kegiatan_ids || []).slice(
      0,
      120,
    ),
...
function baseAuditPayload({ req, correlation_id, action, reason }) {
  return wrapComplianceBase({
    req,
    change_origin: CHANGE_ORIGIN.API_RPJMD_SUB_AUTO_MAP,
    action,
    entity_type: "SUB_KEGIATAN_AUTO_MAP",
    entity_id: null,
    entity_scope_override: "BULK:RPJMD_SUB_AUTO_MAP",
    affected_ids: null,
...
async function preview(req, res) {
  ...
  try {
    const result = await svc.previewSubAutoMap(req.body || {});
    ...
    if (!result.ok) {
      return res.status(400).json({
        success: false,
        message: result.error || "Preview auto mapping sub kegiatan ditolak.",
        data: { correlation_id },
      });
    }
    ...
    console.error("[rpjmdSubAutoMapController.preview]", err);
    ...
    return res.status(500).json({
      success: false,
      message: "Gagal menjalankan preview auto mapping sub kegiatan.",
      data: { correlation_id },
    });
  }
}
...
async function execute(req, res) {
  ...
  try {
    const result = await svc.executeSubAutoMap(req.body || {});
    ...
    if (!result.ok) {
      writeTenantAudit({
        ...
        payload: {
          ...
          request_digest: digestRequest(req.body),
          scanned_count:
            result.data?.preview_summary?.total_sub_kegiatans_scanned ?? 0,
          mapped_count: 0,
          skipped_count: 0,
          error: result.error || "Execute auto mapping sub kegiatan ditolak.",
        },
      });

      return res.status(400).json({
        success: false,
        message: result.error || "Execute auto mapping sub kegiatan ditolak.",
        data: { correlation_id },
      });
    }
    ...
    writeTenantAudit({
      ...
      payload: {
        ...
        change_origin: CHANGE_ORIGIN.API_RPJMD_SUB_AUTO_MAP,
        request_digest: digestRequest(req.body),
        scanned_count: result.data?.preview_summary?.total_sub_kegiatans_scanned ?? 0,
        ...
        affected_ids: (result.data?.mapped_sub_kegiatan_ids || []).slice(0, 200),
    ...
    console.error("[rpjmdSubAutoMapController.execute]", err);
    ...
    return res.status(500).json({
      success: false,
      message: "Gagal menjalankan execute auto mapping sub kegiatan.",
      data: { correlation_id },
    });
  }
}
