...
const { Monev, PeriodeRpjmd } = require("../models");
const Joi = require("joi");

const monevSchema = Joi.object({
...
function toModelPayload(value) {
  ...
  const capaianKinerja =
    cleanText(value.capaian_kinerja) ||
    buildParagraph([
      value.evaluasi,
  ...
  return {
    ...
    kegiatan,
    lokasi: cleanText(value.lokasi),
    capaian_kinerja: capaianKinerja,
    kendala: cleanText(value.kendala),
    tindak_lanjut: cleanText(value.tindak_lanjut) || cleanText(value.rekomendasi),
    jenis_dokumen: cleanText(value.jenis_dokumen),
    pengkeg_id: value.pengkeg_id || null,
  };
}
...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const data = await Monev.findAll({
        where,
  ...
  async getById(req, res) {
    try {
      const { id } = req.params;
      const data = await Monev.findByPk(id, {
        include: [{ model: PeriodeRpjmd, as: "periode" }],
      });
  ...
  async create(req, res) {
    try {
      const { error, value } = monevSchema.validate(req.body);
      if (error)
        return res.status(400).json({ error: error.details[0].message });

      const created = await Monev.create(toModelPayload(value));
  ...
  async update(req, res) {
    try {
      ...
      const [updated] = await Monev.update(toModelPayload(value), { where: { id } });
      if (!updated)
        return res.status(404).json({ error: "Data tidak ditemukan" });

      const result = await Monev.findByPk(id);
      res.json(result);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  ...
  async destroy(req, res) {
    try {
      const { id } = req.params;
      const deleted = await Monev.destroy({ where: { id } });
      if (!deleted)
        return res.status(404).json({ error: "Data tidak ditemukan" });
