...
exports.findByStrategi = async (req, res) => {
  try {
    const { strategi_id, tahun, jenis_dokumen, page = 1, perPage = 50 } = req.query;
    if (!strategi_id) return res.status(400).json({ message: "strategi_id wajib diisi." });
...
exports.getNextKode = async (req, res) => {
  try {
    ...
    if (!strategi) return res.status(404).json({ message: "Strategi tidak ditemukan." });

    const prefix = strategi.kode_strategi || `ST-${strategi_id}`;
...
exports.findOne = async (req, res) => {
  try {
    const record = await IndikatorStrategi.findByPk(req.params.id, {
      include: [
        { model: Strategi, as: "strategi", attributes: ["id", "kode_strategi", "deskripsi"] },
