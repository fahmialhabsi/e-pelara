const { RenstraKebijakan, RenstraOPD, RenstraStrategi, ArahKebijakan } = require('../models');
...
exports.create = async (req, res) => {
  try {
    ...
    if (!chk.ok)
      return res.status(400).json({ error: chk.message, code: chk.code, details: chk.details });
...
exports.generateKodeKebijakan = async (req, res) => {
  try {
    const { arah_kebijakan_id, renstra_id, strategi_id } = req.query;
    ...
    if (strategi_id) {
      // Preferensi: derive dari kode Strategi Renstra LOKAL (sudah unik per baris),
      // bukan dari kode Arah Kebijakan RPJMD (yang bisa dipakai bersama banyak Strategi Renstra).
      const renstraStrategi = await RenstraStrategi.findByPk(strategi_id, {
        attributes: ['id', 'kode_strategi'],
      });
      if (!renstraStrategi?.kode_strategi) {
        return res.status(404).json({ message: 'Strategi Renstra tidak ditemukan.' });
      }
      // Contoh: "SST2-01.03.2.1" -> hapus "SST2-" -> "01.03.2.1" -> ganti titik pertama jadi "-" -> "01-03.2.1"
      const tanpaPrefix = renstraStrategi.kode_strategi.replace(/^SST\d+-/, '');
      bagianAkhir = tanpaPrefix.replace('.', '-');
    } else {
      // Fallback lama (kompatibilitas), jika strategi_id tidak dikirim
      const arahKebijakan = await ArahKebijakan.findByPk(arah_kebijakan_id);
      if (!arahKebijakan) {
        return res.status(404).json({ message: 'Arah Kebijakan tidak ditemukan.' });
      }
      const kodeArah = arahKebijakan.kode_arah;
      bagianAkhir = kodeArah.split('-').slice(1).join('-');
    }

    // Scope suffix berdasarkan strategi_id (jika ada) agar penomoran restart per Strategi Renstra,
    // bukan hanya per Arah Kebijakan RPJMD yang bisa dipakai ulang oleh Strategi Renstra berbeda.
    const whereScope = { renstra_id };
    if (strategi_id) {
      whereScope.strategi_id = strategi_id;
    } else {
      whereScope.rpjmd_arah_id = arah_kebijakan_id;
    }
...
exports.update = async (req, res) => {
  try {
    ...
    if (req.body.strategi_id !== undefined) updateData.strategi_id = req.body.strategi_id;
    if (req.body.rpjmd_arah_id !== undefined) updateData.rpjmd_arah_id = req.body.rpjmd_arah_id;
    if (req.body.deskripsi !== undefined) updateData.deskripsi = req.body.deskripsi;
    if (req.body.prioritas !== undefined) updateData.prioritas = req.body.prioritas;
    if (req.body.isi_arah_rpjmd !== undefined) updateData.isi_arah_rpjmd = req.body.isi_arah_rpjmd;
    if (req.body.jenisDokumen !== undefined) updateData.jenisDokumen = req.body.jenisDokumen;
    if (req.body.tahun !== undefined) updateData.tahun = req.body.tahun;
    if (req.body.renstra_id !== undefined) updateData.renstra_id = req.body.renstra_id;
...
exports.generateKebijakan = async (req, res) => {
  try {
    const { namaOpd, arahKebijakanRpjmd, sasaranRenstra, strategi } = req.body;
    if (!namaOpd || !arahKebijakanRpjmd) {
      return res.status(400).json({ message: 'namaOpd dan arahKebijakanRpjmd wajib diisi' });
    }
    const { generateArahKebijakanRenstra } = require('../services/renstraAIService');
    const hasil = await generateArahKebijakanRenstra({
      namaOpd,
      sasaranRenstra,
      strategi: arahKebijakanRpjmd,
    });
    return res.json({ success: true, kebijakan: hasil });
  } catch (err) {
    console.error('[generateKebijakan]', err);
