...
exports.create = async (req, res) => {
  ...
  try {
    ...
    // Hitung ulang total Kegiatan setelah tambah SubKegiatan
    const kegiatan = await RenstraTabelKegiatan.findByPk(req.body.kegiatan_id, {
      include: [{ model: RenstraTabelSubkegiatan, as: "subKegiatans" }],
      transaction: t,
    });
    const akhir = hitungAkhirKegiatan({ subKegiatans: kegiatan.subKegiatans });
    await kegiatan.update(akhir, { transaction: t });
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    // Hitung ulang total Kegiatan setelah update SubKegiatan
    const kegiatan = await RenstraTabelKegiatan.findByPk(
...
exports.delete = async (req, res) => {
  ...
  try {
    ...
    const kegiatanId = sub.kegiatan_id;
    await sub.destroy({ transaction: t });

    // Hitung ulang total Kegiatan setelah delete SubKegiatan
    const kegiatan = await RenstraTabelKegiatan.findByPk(kegiatanId, {
      include: [{ model: RenstraTabelSubkegiatan, as: "subKegiatans" }],
      transaction: t,
    });
    const akhir = hitungAkhirKegiatan({ subKegiatans: kegiatan.subKegiatans });
