...
const computePaguFinal = (data = {}) => {
  const paguTahun = [1, 2, 3, 4, 5].map(
    (i) => Number(data?.[`pagu_tahun_${i}`]) || 0
  );

  return {
    pagu_akhir_renstra: roundUp2(
      paguTahun.reduce((total, value) => total + value, 0)
    ),
    pagu_tahun_6: 0,
  };
};
...
const buildArahKebijakanPayload = ({
  body,
  current,
  kebijakan,
  indikator,
